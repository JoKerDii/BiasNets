...
def generate_table(datalist, root, seqtype, modelname):
    ...
    for i, f in enumerate(datalist):
        ...
        path = os.path.join(root, seqtype, modelname, filename)
        print("Path: ", path)
        if i == 0:
            myfile = pd.read_pickle(path)
        else:
            myfile = pd.concat([myfile, pd.read_pickle(path)], axis=1)
    pd.to_pickle(myfile, os.path.join(root, seqtype, modelname, "allsamples.pkl"))
...
def plotting(myfile, path):
    ...
    ax.set(
        ylim=(-4, 10),
        xlim=(0, max(myfile.index) + 100000),
        xlabel="transcript",
        ylabel="log(count/count_pred)",
    )
...
if __name__ == "__main__":
    ...
    for seqtype in ["star_RNAseq", "bwa_scRNA"]:
        ...
        for modelname in ["CNN", "MLP"]:
            ...
            # get table
            myfile = pd.read_pickle(
                os.path.join(root, seqtype, modelname, "allsamples.pkl")
            )
            # plot
