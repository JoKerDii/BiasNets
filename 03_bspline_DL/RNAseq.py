"""
# Running example
    python RNAseq_test_copy.py --file=ERR188453Aligned.sortedByCoord.out --readlength=163 --label=count_overlap --model=MLP 
"""
# rnaseq
import argparse
import os
...
def get_data(config):
    ...
    train_loader, valid_loader = data2torch(
        config, train_X, test_X, train_y, test_y, config.batch_size)
    # train_loader = None
    # valid_loader = None
    return train_loader, valid_loader  # train_X, test_X, train_y, test_y


def data2torch(config, train_X, test_X, train_y, test_y, batch_size):
    # load data to torch
    # load train and test
    x_train = torch.tensor(train_X, dtype=torch.float32)
    y_train = torch.tensor(train_y, dtype=torch.float32)
    x_cv = torch.tensor(test_X, dtype=torch.float32)
    y_cv = torch.tensor(test_y, dtype=torch.float32)
...
def training(config, model, train_loader, valid_loader):
    ...
    for epoch in range(config.num_epochs):
        ...
        if config.allow_early_stop == True:
            if epoch > 4 and epochs_no_improve >= config.n_epochs_stop:
                print("Stopping >///<")
                early_stop = True
    ...
    metric_frame = final_test(config, model, valid_loader)
...
if __name__ == "__main__":
    ...
    seq_path = os.path.join(root, "spline-NN", seq)
    ...
    if config.save_mode == "prediction":
        ...
        print("Training ", config.label_iter, " part of data:")
        train_loss, valid_loss, metric_frame = training(
            config, model, valid_loader, train_loader
        )
        print("Metric frame of the ", config.label_iter, " part of data:")
        print(metric_frame)
    else:
