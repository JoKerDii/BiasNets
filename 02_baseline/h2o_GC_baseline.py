...
def get_model_data(all_path, df, label_name):
    ...
    features = df.iloc[:, 1 : 1 + readlength]
    GC_content = pd.DataFrame({"GC": features.ge(2).sum(1) / readlength})
    GC_content

    # append label of interest
    GC_content[label_name] = df[label_name]
    GC_content[label_name] = GC_content[label_name].astype(
        {label_name: "float32"}
    )  # added 1.12

    # transform pandas frame to h2o frame
    train = h2o.H2OFrame(GC_content)
    y = label_name
